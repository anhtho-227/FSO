"""
run_fig3.py -- Tái lập Hình 3 (Section IX.A), 2 chế độ:
  --mode match     : khớp ĐÚNG ĐỘ LỚN như bài báo gốc (dùng cách tính "điều kiện hoá
                      từng mẫu, nhiễu độc lập" -- quantity B -- khớp eq.39 rất sát)
  --mode rigorous  : Helstrom THẬT (density-operator đã sửa lỗi #2+#3, trace-norm
                      chuẩn) -- cho thấy chênh lệch 3-8 lần so với eq.(39) (lỗi #4)

Chạy: python3 experiments/bpsk/run_fig3.py --mode match
      python3 experiments/bpsk/run_fig3.py --mode rigorous
"""
...
def fock_truncated_pure_state_pe(gamma_, alpha, beta, sigma, N, n_samples, seed):
    """Cach tinh 'quantity B': dieu kien hoa tung mau (I1, nhieu DOC LAP cho 2 gia
    thuyet), xay trang thai THUAN KHIET cat cut Fock N chieu, Helstrom cho CAP
    trang thai thuan khiet do, roi trung binh -- day la cach khien do lon khop
    voi Hinh 3 cua bai bao (xem ERRORS.md, muc "Vi sao V2 khong phat hien loi #4")."""
...
def run_match(alpha, beta, sigma, gammas, outpath):
    ...
    ax.set_title(f'Hình 3 [match] — α={alpha}, β={beta}, σ={sigma}')
...
def run_rigorous(alpha, beta, sigma, gammas, outpath, N=24):
    ...
    ax.semilogy(gammas, pe_true, '-s', color='black', linewidth=2, label=f'Helstrom THẬT (density-op đã sửa, N={N})')
    ax.set_xlabel('|γ|'); ax.set_ylabel('Helstrom Error Probability (log)')
    ax.set_title(f'Hình 3 [rigorous] — α={alpha}, β={beta}, σ={sigma}\n(cho thấy lỗi #4: eq.39 thấp hơn 3-8 lần)')
    ax.set_ylim(1e-4, 1e0); ax.grid(True, which='both', ls=':', alpha=0.5)
    ax.legend(fontsize=8)
    fig.tight_layout(); fig.savefig(outpath, dpi=160); plt.close(fig)
